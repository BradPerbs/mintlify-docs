...
You can connect via **local (stdio)** using the npm package, or via the **hosted SSE endpoint** at `https://mcp.geogen.io/sse`.

## Prerequisites

- **Node.js** v18 or higher (for stdio transport only)
...
<Tabs>
  ...
  <Tab title="OpenAI Agent Builder">
    In the OpenAI Agent Builder, add a new MCP tool with the following configuration:
